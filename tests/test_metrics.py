...
def dummy_cfg():
    ...
    Also include BATCH_KEY in the config as a list so that its values can be accumulated.
    """
    return SimpleNamespace(metrics={
        "loss": "int",  # MetricType.INT.value
        "preds": "list",  # MetricType.LIST.value
        "weighted": "batch_weighted_avg_list",  # MetricType.BATCH_WEIGHTED_AVG_LIST.value
...
def test_metrics_group_add_dict(dummy_cfg):
    ...
    mg.add({"loss": 2, "preds": "b", "weighted": 30}, ns=2)
    # For "loss": initial 0 + 2 = 2; for "preds": [] becomes ["b"];
    # for "weighted": [] becomes [30]; and a batch value of 2 is appended.
    assert mg.data["loss"] == 2
    assert mg.data["preds"] == ["b"]
    assert mg.data["weighted"] == [30]
    assert mg.data[BATCH_KEY] == [2]
...
def test_metrics_group_no_config():
    # When cfg.metrics is None, the MetricsGroup should initialize with an empty data dict.
